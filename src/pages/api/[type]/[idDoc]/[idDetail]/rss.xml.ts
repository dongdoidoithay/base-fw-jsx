...
export default async (req: any, res: any) => {
  try {
    const { type, idDoc,idDetail } = req.query;
    ...
    if (current &&
        ...
        const start_chapter_prefix = current.uri_select.start_view;
        _idDetail = idDetail.toString().replace(start_chapter_prefix, "");

        const dataInfos = [
            `${Constants.UrlApi}${current.uri_select.prefix_url_api}${Constants.path_view}/${_idDoc}/${_idDetail}`,
        ];
    ...
    if (_data_info[0]?.data?.infoDoc) {
      ...
      url = `https://${domain}${UrlDetail(
        _data_info[0]?.data?.infoDoc.idDoc,
        idDetail,
        current
      )}`;
      urlApi = `https://${domain}/api${UrlDetail(
        _data_info[0]?.data?.infoDoc.idDoc,
        idDetail,
        current
      )}`;
    ...
    const manga_rend = _data_info[0].data.chapterList
      .map((data: any) => {
