...
export const getServerSideProps = (async (context) => {
  ...
  const dataUrls = [
    `${Constants.UrlApi}${_current.uri_select.prefix_url_api}${Constants.path_page_type}/${TypeMode.Years}/${_key}/24/0`,
  ];
...
export default function LastUpdate({
  repo
    ...
    const path= `${Constants.UrlApi}${repo?.current.uri_select.prefix_url_api}${Constants.path_page_type}/${TypeMode.Years}/${repo?.key}/24`;
  //console.log(repo);
  return (
    <>
      ...
      <BlockList current={repo?.current} data={repo?.data} name={`${repo?.current?.lable_info.info_year} - ${repo?.key}`} path={path}/>
      <Ads_Block_Banner current={repo?.current}/>
