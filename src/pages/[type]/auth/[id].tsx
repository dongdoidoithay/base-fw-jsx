...
export const getServerSideProps = (async (context) => {
  ...
  const dataUrls = [
    `${Constants.UrlApi}${_current.uri_select.prefix_url_api}${Constants.path_page_type}/${TypeMode.Auths}/${_key}/24/0`,
  ];
...
export default function LastUpdate({
  repo
    ...
    const path= `${Constants.UrlApi}${repo?.current.uri_select.prefix_url_api}${Constants.path_page_type}/${TypeMode.Auths}/${repo?.key}/24`;
  //console.log(repo);
  return (
    <>
      <Head>
      ...
      <BlockList current={repo?.current} data={repo?.data} name={`${repo?.current?.lable_info.info_auth} - ${repo?.key}`} path={path}/>
